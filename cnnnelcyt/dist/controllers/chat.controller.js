...
exports.updateMemberStatus = exports.removeMember = exports.getChatMembers = exports.getChats = exports.createChat = exports.addMember = exports.patchChat = exports.getChatById = exports.getPublicChats = void 0;
...
const patchChat = async (req, res) => {
    ...
    try {
        ...
        const memberCheck = await (0, db_1.query)('SELECT role FROM chat_members WHERE chat_id = $1 AND user_id = $2', [id, userId]);
        if (memberCheck.rows.length === 0) {
            res.status(403).json({ error: 'Not a member of this chat' });
            return;
        }
        await (0, db_1.query)(`UPDATE chats SET 
        name = COALESCE($1, name),
        description = COALESCE($2, description),
        avatar_url = COALESCE($3, avatar_url),
        cover_url = COALESCE($4, cover_url),
        is_public = COALESCE($5, is_public)
       WHERE id = $6`, [name ?? null, description ?? null, avatar_url ?? null, cover_url ?? null, is_public ?? null, id]);
        const updated = await (0, db_1.query)('SELECT * FROM chats WHERE id = $1', [id]);
...
const addMember = async (req, res) => {
    ...
    try {
        await (0, db_1.query)(`INSERT INTO chat_members (chat_id, user_id, role, joined_at)
       VALUES ($1, $2, 'member', NOW())
       ON CONFLICT (chat_id, user_id) DO UPDATE SET role = 'member'`, [id, userId]);
...
const createChat = async (req, res) => {
    ...
    try {
        ...
        if (is_group === false && memberIds && memberIds.length === 1) {
            ...
            const existingDM = await (0, db_1.query)(`SELECT c.id FROM chats c
         JOIN chat_members cm1 ON c.id = cm1.chat_id
         JOIN chat_members cm2 ON c.id = cm2.chat_id
         WHERE c.is_group = false 
         AND cm1.user_id = $1 AND cm2.user_id = $2`, [userId, otherUserId]);
        ...
        const chatResult = await (0, db_1.query)('INSERT INTO chats (name, description, avatar_url, is_group) VALUES ($1, $2, $3, $4) RETURNING id', [name, description, avatar_url, is_group]);
        const chatId = chatResult.rows[0].id;
        // Add creator as admin
        await (0, db_1.query)('INSERT INTO chat_members (chat_id, user_id, role) VALUES ($1, $2, $3)', [chatId, userId, 'admin']);
        // Add other members
        if (memberIds && Array.isArray(memberIds)) {
            for (const id of memberIds) {
                if (id !== userId) {
                    await (0, db_1.query)('INSERT INTO chat_members (chat_id, user_id, role) VALUES ($1, $2, $3)', [chatId, id, 'member']);
                }
            }
        }
...
const getChatMembers = async (req, res) => {
    ...
    try {
        const result = await (0, db_1.query)(`SELECT p.id, p.name, p.avatar_url, p.bio, p.job_role, p.college_name,
              p.status, p.last_seen, cm.role, cm.joined_at 
       FROM chat_members cm
       JOIN profiles p ON cm.user_id = p.id
       WHERE cm.chat_id = $1
       ORDER BY cm.joined_at ASC`, [chatId]);
        res.status(200).json(result.rows);
    }
    catch (error) {
        console.error(error);
...
const removeMember = async (req, res) => {
    ...
    try {
        ...
        await (0, db_1.query)('DELETE FROM chat_members WHERE chat_id = $1 AND user_id = $2', [id, userId]);
...
const updateMemberStatus = async (req, res) => {
    ...
    try {
        ...
        await (0, db_1.query)('UPDATE chat_members SET status = $1 WHERE chat_id = $2 AND user_id = $3', [status, id, userId]);
        res.status(200).json({ success: true });
    }
    catch (error) {
        console.error(error);
