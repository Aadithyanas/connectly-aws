...
async function run() {
    ...
    try {
        const result = await index_1.default.query('SELECT id, name FROM public.profiles WHERE email = $1', [email]);
        ...
        const token = jsonwebtoken_1.default.sign({ id: user.id, email }, JWT_SECRET, { expiresIn: '365d' }); // Valid for 1 year
